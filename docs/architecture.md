...
Shutter centralizes derivative concerns that would otherwise be duplicated across
...
flowchart LR
  ...
  video --> derived["Shutter Derivative Store on R2"]
  pdf --> derived
...
Cloudflare is Shutter's delivery edge and durable Derivative storage provider. A
Cloudflare Worker decrypts and validates private Source Capabilities before
every data-center-local edge-cache lookup and can read stored Derivatives through
an R2 binding; ordinary Cloudflare CDN caching serves canonical public
Derivatives. Railway hosts Shutter Control, imgproxy, the isolated Executors, and
...
on Node with Hono; Drizzle manages the Postgres job schema; R2 is the Derivative
...
Its only durable binding is the R2 Derivative Store. Jobs and operational metadata
remain in Postgres on Railway.

Postgres contains the Space Registry and the `derivative_jobs` ledger. A registry
...
| Media identity and source-to-derivative relationships | Consuming application |
| Space policy configuration, Derivative Jobs, attempts, retries | Shutter |
| Image Optimization | imgproxy, configured by Shutter |
| Generated and cached Derivative bytes | Shutter Derivative Store |
...
Derivative Store. Source Delivery can stream an allowlisted Source Object through
the Worker and keep a complete object in the ephemeral Cloudflare cache. A
Derivative Job may retain the source reference and output metadata needed for
...
Derivative Store keys, and Source Purge. A Source Locator supplies only the
...
Shutter owns a separate Derivative Store containing only generated Derivative
bytes. Object keys are deterministic from the Shutter Space, Source ID,
derivative kind, and normalized parameters. Applications retain the
meaningful relationship between their media records and returned Derivative
references. Private reads pass through the Shutter authorization gateway.

The v1 Derivative Store is Cloudflare R2. The Worker reads it through a native R2
...
variant, stored Derivative, and operational Derivative Job for that Source ID;
...
capability can recreate a derivative while its Source Locator remains fetchable.

All Cloudflare cache entries for a source carry a tag derived from a hash of the
Space ID and Source ID. R2 keys group cached derivatives and Master Previews under
...
concurrent edge request cannot repopulate a purged cache from a stored derivative.
...
2. The frontend combines that source reference with permitted derivative
   parameters in a stateless Derivative URL; it does not call Shutter to mint the
   URL.
...
normalized derivative parameters, so a refreshed Source Capability
...
Worker clones an authorized derivative into its canonical Cache API key with a
...
the Derivative Store. Unpic and imgproxy then produce normalized responsive image
...
including Space, Source ID, purpose, derivative kind, expiry, and origin policy.
...
ready Master Preview or a Derivative Job reference. Applications poll that
