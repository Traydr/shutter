...
Source ID, one strict purpose, and expiry. `image_source`, `source_delivery`, and
`preview_job` carry an allowlisted HTTPS Source Locator; `master_preview`
identifies an existing stored kind without an original locator. Shutter alone
can read or validate the claims, and visible key identifiers permit overlapping
rotation.
