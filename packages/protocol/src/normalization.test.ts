...
import { normalizeOptimizationQuery, normalizeQuality, normalizeWidth } from "./normalization.js";

const policy = { qualities: [30, 50, 75] as const, defaultQuality: 75 };

describe("optimization query normalization", () => {
  it.each([
    [24, 24],
    [1, 320],
    [23, 320],
  ...
  it("defaults only quality and reports whether the request is canonical", () => {
    expect(normalizeOptimizationQuery(new URLSearchParams("w=640"), policy)).toEqual({
      width: 640,
      quality: 75,
      isCanonical: false,
    });
    expect(normalizeOptimizationQuery(new URLSearchParams("w=640&q=75"), policy)).toEqual({
      width: 640,
      quality: 75,
      isCanonical: true,
    });
    expect(normalizeOptimizationQuery(new URLSearchParams("w=500&q=74"), policy)).toEqual({
      width: 640,
      quality: 75,
      isCanonical: false,
    });
  ...
  it.each([
    ...
    expect(() => normalizeOptimizationQuery(new URLSearchParams(query), policy)).toThrow(
      ProtocolError,
    );
  });
});
