...
function spikeUrl(): string {
  const url = new URL("http://shutter.test/internal/v1/spike/derivative");
...
describe("control app", () => {
  ...
  it("rejects incomplete or malformed derivative requests before imgproxy", async () => {
    const fetch = vi.fn();
    const control = createControlApp({
      logger: NOOP_LOGGER,
      spaceRegistry: SPACE_REGISTRY,
    ...
    const missingSource = await control.request(
      "http://shutter.test/internal/v1/spike/derivative?key=cache/v1/private/example-private/fp/source/w640-q75.webp&w=640&q=75",
      { headers: { authorization: `Bearer ${TOKEN}` } },
    );
  ...
  it("returns unavailable when Postgres is not configured", async () => {
    ...
    const derivative = await control.request(spikeUrl(), {
      headers: { authorization: `Bearer ${TOKEN}` },
    });
    ...
    expect(derivative.status).toBe(503);
  ...
  it("authenticates and strictly validates master derivative requests", async () => {
    ...
    const url = "http://shutter.test/internal/v1/master-derivative";
  ...
  it("does not expose a presigned master URL when imgproxy fails", async () => {
    ...
    const response = await control.request("http://shutter.test/internal/v1/master-derivative", {
      method: "POST",
      headers: { authorization: `Bearer ${TOKEN}`, "content-type": "application/json" },
      body: JSON.stringify({
        spaceId: "example-private",
  ...
  it("issues a server-generated request ID and ignores caller-controlled ones", async () => {
    ...
    const response = await control.request("http://shutter.test/internal/v1/spike/derivative", {
  ...
  it("creates a Space through admin, observes Edge refresh, and renders without code configuration", async () => {
    ...
    const derivativeUrl = new URL("https://shutter.test/internal/v1/spike/derivative");
    derivativeUrl.searchParams.set(
      "key",
      "cache/v1/public/admin-created/fingerprint/source/w640-q75.webp",
    );
    derivativeUrl.searchParams.set("source", "https://sources.example.com/media/image.jpg");
    derivativeUrl.searchParams.set("w", "640");
    derivativeUrl.searchParams.set("q", "75");
    const derivative = await control.request(derivativeUrl, {
      headers: { authorization: `Bearer ${TOKEN}` },
    });
