...
import type { DerivativeJobLifecycle, SourceIdentity } from "./derivative-job-lifecycle.js";
...
async function deletePrefix(s3: S3Client, bucket: string, prefix: string): Promise<void> {
  ...
  do {
    ...
    if (objects.length > 0) {
      ...
      if ((deleted.Errors?.length ?? 0) > 0) throw new Error("derivative deletion failed");
    }
    continuationToken = page.IsTruncated ? page.NextContinuationToken : undefined;
    if (page.IsTruncated && continuationToken === undefined)
      throw new Error("derivative listing pagination failed");
