...
describe("Source Purge", () => {
  ...
  it("deletes every paginated prefix before purging Worker and zone cache tags", async () => {
    const identity = { spaceId: "demo-private", sourceId: "source/one", kind: "video" as const };
    ...
    const events: string[] = [];
    let publicPages = 0;
    const tag = await buildSourceCacheTag("demo-private", "source/one");
    ...
    await sourcePurge.purge({ spaceId: "demo-private", sourceId: "source/one" });
  ...
  it("stops before later purge stages when an earlier stage fails", async () => {
    ...
    await expect(
      deleteFails.purge({ spaceId: "demo-private", sourceId: "source" }),
    ...
    await expect(
      workerFails.purge({ spaceId: "demo-private", sourceId: "source" }),
  ...
  it("retries safely after a Cloudflare failure", async () => {
    const identity = { spaceId: "demo-private", sourceId: "source", kind: "pdf" as const };
    await lifecycle.submit(
      {
        ...identity,
        sourceCapability: "opaque",
    ...
    const sourcePurge = createPurge(fetch, send);
    await expect(
      sourcePurge.purge({ spaceId: "demo-private", sourceId: "source" }),
