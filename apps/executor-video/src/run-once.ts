...
export interface VideoExecutorConfig {
...
export async function runVideoOnce(config: VideoExecutorConfig): Promise<"idle" | "processed"> {
  const claimed = await control(config, "/internal/v1/executors/video/claim", { method: "POST" });
  ...
  const directory = await mkdtemp(join(tmpdir(), "shutter-video-"));
  let uploaded = false;
  const transition = `/internal/v1/executors/video/jobs/${encodeURIComponent(claim.spaceId)}/${encodeURIComponent(claim.sourceId)}`;
  ...
  try {
    const preview = await processVideoPreview(
      claim.locator,
