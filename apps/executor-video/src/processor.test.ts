...
describe("video preview processor", () => {
  it.effect("downloads over HTTPS, renders a bounded WebP, and reports dimensions", () =>
    Effect.gen(function* () {
      const { input, output } = yield* Effect.promise(paths);
      const fetch_ = vi.fn(
        async () =>
      ...
      const result = yield* processVideoPreview("https://media.example/video.mp4", input, output, {
        fetch: fetch_,
        runCommand: run,
  ...
  it.effect("rejects a redirect that downgrades to HTTP before fetching it", () =>
    Effect.gen(function* () {
      const { input, output } = yield* Effect.promise(paths);
      const fetch_ = vi.fn(
        async () =>
      ...
      expect(error).toMatchObject<Partial<ProcessingFailure>>({
        code: "source_missing",
        retryable: false,
      });
      expect(fetch_).toHaveBeenCalledOnce();
  ...
  it.effect("rejects a redirect off the Space allowlist", () =>
    Effect.gen(function* () {
      const { input, output } = yield* Effect.promise(paths);
      const fetch_ = vi.fn(
        async () =>
